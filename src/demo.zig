...
fn randomCssColour() []const u8 { return css_colours[nextRandom() % css_colours.len]; }
fn randomCssSize()   []const u8 { return css_sizes[nextRandom()   % css_sizes.len];   }
...
fn drawGradientFilledRectangles(h: i32) void {
    ...
    while (i < 15) : (i += 1) {
        ...
        canvasOne.colourFilledRectangle(
            15, 10 + @divTrunc(h, 2) + step * 17, 15, 15,
...
fn drawGlowingHalos(ctx: dom.Context2D, w: i32, h: i32) void {
    ...
    ctx.beginPath(); ctx.fillStyle("rgba(255,0,180,0.18)");  ctx.arc(cx, cy, 70.0, 0.0, 6.2832, false); ctx.fill();
    ctx.beginPath(); ctx.fillStyle("rgba(0,240,220,0.28)");  ctx.arc(cx, cy, 48.0, 0.0, 6.2832, false); ctx.fill();
    ctx.beginPath(); ctx.fillStyle("rgba(255,180,30,0.55)"); ctx.arc(cx, cy, 26.0, 0.0, 6.2832, false); ctx.fill();
    ctx.beginPath(); ctx.fillStyle("rgba(255,60,200,0.90)"); ctx.arc(cx, cy, 10.0, 0.0, 6.2832, false); ctx.fill();
}
...
fn updateCanvasTwo() void {
    ...
    for (&balls) |*ball| {
        ...
        if (ball.x - r < 0.0)  { ball.x = r;     ball.vx =  @abs(ball.vx) * dampen; }
        if (ball.x + r > w)    { ball.x = w - r;  ball.vx = -@abs(ball.vx) * dampen; }
        if (ball.y - r < 0.0)  { ball.y = r;      ball.vy =  @abs(ball.vy) * dampen; }
        if (ball.y + r > h)    { ball.y = h - r;  ball.vy = -@abs(ball.vy) * dampen; }
